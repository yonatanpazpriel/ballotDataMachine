...
> = {
  P: {
    direct: PROSECUTION_KEYS.filter((k) => k.includes("Direct") && k.includes("Attorney")),
    cross: PROSECUTION_KEYS.filter((k) => k.includes("Cross") && k.includes("Attorney")),
    statement: PROSECUTION_KEYS.filter((k) => k.includes("Open") || k.includes("Close")),
    witnessDirect: PROSECUTION_KEYS.filter((k) => k.includes("Direct") && k.includes("Witness")),
    witnessCross: PROSECUTION_KEYS.filter((k) => k.includes("Cross") && k.includes("Witness")),
  },
  D: {
    direct: DEFENSE_KEYS.filter((k) => k.includes("Direct") && k.includes("Attorney")),
    cross: DEFENSE_KEYS.filter((k) => k.includes("Cross") && k.includes("Attorney")),
    statement: DEFENSE_KEYS.filter((k) => k.includes("Open") || k.includes("Close")),
    witnessDirect: DEFENSE_KEYS.filter((k) => k.includes("Direct") && k.includes("Witness")),
    witnessCross: DEFENSE_KEYS.filter((k) => k.includes("Cross") && k.includes("Witness")),
  },
};
...
function ingestScores(ballots: Ballot[], side: OurSide) {
  ...
  for (const ballot of getSideBallots(ballots, side)) {
    ...
    const openerPickup = openerScore && oppOpenerScore ? openerScore.score - oppOpenerScore.score : null;
    const closerPickup = closerScore && oppCloserScore ? closerScore.score - oppCloserScore.score : null;
...
function buildRoles(ballots: Ballot[], side: OurSide): RoleResult[] {
  ...
  if (roleNames.opener) roles.push({ name: roleNames.opener, stats: stats[roleNames.opener] || emptyStats });
  if (roleNames.middle) roles.push({ name: roleNames.middle, stats: stats[roleNames.middle] || emptyStats });
  if (roleNames.closer) roles.push({ name: roleNames.closer, stats: stats[roleNames.closer] || emptyStats });

  roleNames.witnesses.forEach((w) => {
    roles.push({ name: w, stats: stats[w] || emptyStats });
  });
